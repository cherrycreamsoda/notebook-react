...
const DatasheetEditor = ({ content, onContentChange }) => {
  ...
  useEffect(() => {
    if (content && typeof content === "object" && content.rows) {
      ...
      // Start with one empty row for new tables
      const initialRow = {
        id: generateId(),
        cells: [
      ...
      // Auto-focus first cell for new tables
      setTimeout(() => {
  ...
  const removeRow = (rowId) => {
    ...
    // Clear selection if removing selected row
  ...
  const handleCellKeyDown = (e, rowId, columnId) => {
    if (e.key === "Enter") {
      ...
      e.preventDefault();
      // Same logic as Enter but without creating new rows
      const currentRowIndex = data.rows.findIndex((r) => r.id === rowId);
      const currentColumnIndex = data.columns.findIndex(
        (c) => c.id === columnId
      );
