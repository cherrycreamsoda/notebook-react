...
const GlassmorphicFAB = ({ onCreateNote, selectedNote, sidebarCollapsed }) => {
  ...
  const handleMouseUp = (e) => {
    ...
    // If not expanded, create default note
    if (!isExpanded) {
      onCreateNote();
    }
  };
  ...
  const handleTypeSelect = async (noteType) => {
    // Create note with specific type and proper content structure
    const noteData = {
      type: noteType.value,
      title: "New Note",
      content:
    ...
    // Close expansion after selection
    setIsExpanded(false);
  };
