...
const RemindersEditor = ({ content, onContentChange }) => {
  ...
  // Initialize reminders from content
  useEffect(() => {
    if (
      content &&
  ...
  // Update content when reminders change
  useEffect(() => {
  ...
  const removeReminder = (id) => {
    if (reminders.length <= 1) return; // Keep at least one reminder
  ...
  const getReminderStatus = (reminder) => {
    ...
    if (reminderTime - now < 60 * 60 * 1000) return "soon"; // Within 1 hour
    return "pending";
  };
  ...
  const sortedReminders = [...reminders].sort((a, b) => {
    ...
    // Sort by datetime if both have it
    if (a.datetime && b.datetime) {
      return new Date(a.datetime) - new Date(b.datetime);
    }

    // Items with datetime come first
